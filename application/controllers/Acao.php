...
class Acao extends CI_Controller {
	...
	public function __construct()
	{
        ...
        $this->load->model(array('acao_model'));
	}
    ...
    public function index()
    {
        ...
        $data['title'] = 'acao';
        $data['err_form'] = '';
        $data['acao'] = $this->acao_model->get_acao();

        $this->load->view('templates/header.php', $data);
        $this->load->view('templates/nav.php', $data);
        $this->load->view('acao/acao.php', $data);
    ...
    public function inserir()
    {
        ...
        $config_form_validation = array(
            array(
                'field' => 'acao_nome',
                'label' => 'Nome',
                'rules' => 'trim|stripslashes|htmlspecialchars|encode_php_tags|required|min_length[3]|max_length[45]|is_unique[acao.acao_nome]'
            ),
            array(
                'field' => 'acao_descricao',
                'label' => 'Descrição',
        ...
        if(!empty($_POST))
        {
            ...
            {
                if($last_id = $this->acao_model->set_acao($_POST))
                {
                    redirect('acao');
                }
                else
        ...
        $this->load->view('templates/header.php', $data);
        $this->load->view('templates/nav.php', $data);
        $this->load->view('acao/acao_inserir.php', $data);
        $this->load->view('templates/footer.php', $data);
    }
    ...
    public function editar($id)
    {
        ...
        $data['title'] = 'acao';
        $data['err_form'] = '';
        $data['acao'] = $this->acao_model->get_acao($id);
        ...
        if(!empty($_POST))
        {
            ...
            if($this->form_validation->run())
            {
                if($last_id = $this->acao_model->update_acao($_POST, $id))
                {
                    redirect('acao');
                }
                else
        ...
        $this->load->view('templates/header.php', $data);
        $this->load->view('templates/nav.php', $data);
        $this->load->view('acao/acao_editar.php', $data);
        $this->load->view('templates/footer.php', $data);
    }
    ...
    public function check_exists($param_1, $id)
    {
        ...
        if(!$this->acao_model->get_acao_nome_exists($param_1, $id) == 0)
        {
