...
class Empresa extends CI_Controller {
    ...
    public function index()
    {
        ...
        $this->load->view('empresa/empresa.php', $data);
    ...
    public function inserir()
    {
        ...
        $config_form_validation = array(
            array(
                'field' => 'empresa_nome',
                'label' => 'Nome',
                'rules' => 'trim|stripslashes|htmlspecialchars|encode_php_tags|required|min_length[3]|max_length[45]|is_unique[empresa.empresa_nome]'
            ),
            array(
                'field' => 'empresa_descricao',
                'label' => 'Descrição',
                'rules' => 'trim|stripslashes|htmlspecialchars|encode_php_tags|min_length[3]|max_length[245]'
            )
        );
        ...
        if(!empty($_POST))
        {
            ...
            {
                if($last_id = $this->empresa_model->set_empresa($_POST))
                {
                    redirect('empresa');
                }
                else
        ...
        $this->load->view('templates/header.php', $data);
        $this->load->view('templates/nav.php', $data);
        $this->load->view('empresa/empresa_inserir.php', $data);
        $this->load->view('templates/footer.php', $data);
    }
    ...
    public function editar($id)
    {
        ...
        if(!empty($_POST))
        {
            ...
            if($this->form_validation->run())
            {
                if($last_id = $this->empresa_model->update_empresa($_POST, $id))
                {
                    redirect('empresa');
                }
                else
        ...
        $this->load->view('templates/header.php', $data);
        $this->load->view('templates/nav.php', $data);
        $this->load->view('empresa/empresa_editar.php', $data);
        $this->load->view('templates/footer.php', $data);
    }
    ...
    public function check_exists($param_1, $id)
    {
        ...
        if(!$this->empresa_model->get_empresa_nome_exists($param_1, $id) == 0)
        {
